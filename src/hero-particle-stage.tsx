...
function createBrandMask(ctx: CanvasRenderingContext2D, width: number, height: number, focusPoints: readonly FocusPoint[]) {
  const centerX = width * 0.5;
  ...
  const orbitGradient = ctx.createLinearGradient(centerX - orbitWidth, centerY - orbitHeight, centerX + orbitWidth, centerY + orbitHeight);
  ...
  ctx.beginPath();
  ctx.ellipse(centerX, centerY, orbitWidth, orbitHeight, -0.12, Math.PI * 0.08, Math.PI * 1.92);
  ctx.stroke();

  ctx.beginPath();
  ctx.ellipse(centerX, centerY, orbitWidth * 0.72, orbitHeight * 0.56, 0.18, Math.PI * 0.14, Math.PI * 1.84);
  ctx.stroke();

  ctx.fillStyle = 'rgba(226, 232, 240, 0.88)';
  focusPoints.forEach((point, index) => {
    ctx.beginPath();
    ctx.arc(point.x, point.y, width * (index % 2 === 0 ? 0.009 : 0.007), 0, Math.PI * 2);
    ctx.fill();
  });

  const textGradient = ctx.createLinearGradient(centerX - width * 0.18, centerY, centerX + width * 0.18, centerY);
...
function extractSamples(maskCanvas: HTMLCanvasElement, focusPoints: readonly FocusPoint[]) {
  ...
  return samples.filter((_, index) => {
    ...
    return focusPoints.some((focus) => distance(point.x, point.y, focus.x, focus.y) < width * 0.04);
  });
}
...
class HeroParticleEngine {
  ...
  private rebuild() {
    ...
    this.particles = samples.map((sample, index) => {
      ...
      const focusIndex = this.focusPoints.reduce((closest, point, pointIndex) => {
        const currentDistance = distance(sample.x, sample.y, point.x, point.y);
        const closestDistance = distance(sample.x, sample.y, this.focusPoints[closest].x, this.focusPoints[closest].y);
  ...
  private update(time: number, delta: number) {
    ...
    this.particles.forEach((particle) => {
      if (swirlWeight > 0.02) {
        ...
        const swirlDistance = Math.max(distance(particle.x, particle.y, swirlX, swirlY), 1);
        const swirlFalloff = clamp(1 - swirlDistance / Math.max(this.canvas.width, this.canvas.height), 0, 1);
        const radialForce = swirlFalloff * swirlFalloff * 0.06 * swirlWeight * delta;
      ...
      particle.vx += (backX / (20 * particle.mass)) * (0.5 + introProgress) * delta;
      particle.vy += (backY / (20 * particle.mass)) * (0.5 + introProgress) * delta;

      const driftX = Math.cos(time * 0.0008 + particle.phase) * particle.drift * 0.018;
      const driftY = Math.sin(time * 0.0011 + particle.phase * 1.2) * particle.drift * 0.014;
      ...
      this.bursts.forEach((burst) => {
        ...
        const force = ((burstRadius - burstDistance) / burstRadius) * burstProgress * 1.65 * burst.strength;
  ...
  private drawHalo(time: number, opacity = 1) {
    ...
    const halo = this.ctx.createRadialGradient(centerX, centerY, haloRadius * 0.08, centerX, centerY, haloRadius);
    ...
    this.focusPoints.forEach((point, index) => {
      ...
      const glow = this.ctx.createRadialGradient(point.x, point.y, 0, point.x, point.y, radius);
  ...
  private drawConnections(time: number, opacity = 1) {
    ...
    this.particles.forEach((particle) => {
      ...
      const alpha = clamp((0.06 + Math.sin(time * 0.0018 + particle.phase) * 0.025) * opacity, 0.02, 0.12);
  ...
  private drawParticles(time: number, opacity = 1) {
    ...
    this.particles.forEach((particle) => {
      ...
      this.ctx.fillRect(particle.x - particle.size * 0.5, particle.y - particle.size * 0.5, particle.size, particle.size);
    });
...
export function HeroParticleStage() {
  ...
  useEffect(() => {
    ...
    const motionMedia = globalThis.matchMedia('(prefers-reduced-motion: reduce)');
    ...
    const pointerFromEvent = (event: PointerEvent) => {
      ...
      engine.setPointer(event.clientX - bounds.left, event.clientY - bounds.top);
    };
  ...
  return <canvas className="hero-particle-stage" aria-hidden="true" ref={canvasRef} />;
}
